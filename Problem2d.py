...
class TestPasswordFile(unittest.TestCase):
    ...
    def test_add_new_user(self):
        ...
        add_user("testuser", "TestPass123!", self.test_file, "Client")
        with open(self.test_file, "r") as f:
            content = f.read()
        self.assertIn("testuser", content)
        self.assertTrue(content.strip().count(":") == 3)  # Ensure format is correct

    def test_prevent_duplicate_usernames(self):
        # Add a user and attempt to add the same username again
        add_user("testuser", "TestPass123!", self.test_file, "Client")
        with self.assertRaises(ValueError) as context:
            add_user("testuser", "AnotherPass123!", self.test_file, "Client")
        self.assertEqual(str(context.exception), "Username already exists.")

    def test_verify_correct_password(self):
        # Add a user and verify with the correct password
        add_user("testuser", "TestPass123!", self.test_file, "Client")
        self.assertTrue(verify_user("testuser", "TestPass123!", self.test_file, "Client"))

    def test_verify_incorrect_password(self):
        # Add a user and verify with an incorrect password
        add_user("testuser", "TestPass123!", self.test_file, "Client")
        self.assertFalse(verify_user("testuser", "WrongPass123!", self.test_file, "Client"))

    def test_verify_nonexistent_user(self):
        # Attempt to verify a user that doesn't exist
        self.assertFalse(verify_user("nonexistentuser", "RandomPass123!", self.test_file, "Client"))

    def test_unique_salt_for_users_with_same_password(self):
        # Add two users with the same password
        add_user("user1", "SamePassword123!", self.test_file, "Client")
        add_user("user2", "SamePassword123!", self.test_file, "Client")
        with open(self.test_file, "r") as f:
            lines = f.readlines()
